# -*- coding: utf-8 -*-
"""merged_coco_commu.json から CoCo夏夏夏 Holiday のイベントコミュHTMLを生成する

出力: scripts/coco_commu_section.html（ページに手動で組み込む用の断片）
"""
import html
import json
import sys
...
MERGED = REPO / "scripts" / "merged_coco_commu.json"
OUT = REPO / "scripts" / "coco_commu_section.html"

CDN = "https://res.cloudinary.com/dnmzdghoi/image/upload/f_auto,q_auto/Deresute/Event/CoCoNatsuHoliday"
...
def main():
    ...
    for i, sec in enumerate(data):
        ...
        tabs_html.append(
            f'        <button class="tab-item{active}" onclick="switchTab(this,\'csh-{sec["id"]}\')">{esc(sec["tab"])}</button>')
        ...
        rows_html = "\n        ".join(rows)
        panels_html.append(f'''    <div class="tab-panel{active}" data-tab="csh-{sec["id"]}">
