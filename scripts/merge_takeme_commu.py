# -*- coding: utf-8 -*-
"""Take me☆Take you イベントコミュのCSV×JSONマージスクリプト

- dialogues.csv: スクショのフレーム・タイムスタンプ・センターテキストを保持
- *_log.json:    完全なセリフテキスト・タイトル・要約を保持

両者を突合し、タブ生成用のマージ済みJSON (merged_takeme_commu.json) と
突合レポート (merge_takeme_report.txt) を出力する。
"""
import csv
import json
import re
...
CSV_PATH = Path(r"C:\Users\sawas\Downloads\comm_frames\Take me☆Take you_202606222304\dialogues.csv")
JSON_DIR = Path(r"G:\マイドライブ\コミュ")
JSON_PREFIX = "Take me☆Take you"
OUT_DIR = Path(__file__).parent
OUT_JSON = OUT_DIR / "merged_takeme_commu.json"
OUT_REPORT = OUT_DIR / "merge_takeme_report.txt"

# 場所・時間を示すセンターテキスト（タイトルカード扱い）
LOCATIONS = {
    "ラジオブース", "控え室", "コンビニ前", "事務所",
    "撮影スタジオ", "撮影後", "LIVE会場",
}
...
KNOWN_SPEAKERS = {"芳乃", "美優", "乃々", "みく", "心", "P"}
...
def load_json_lines(code):
    ...
    for c in d["conversation"]:
        ...
        if sp == dg:
            ...
            continue
        if sp == "（テキストなし）":
            # 話者不在の地の文ナレーション。CSVのセンターテキスト（ヘッダ）に
            # 同一テキストが入っているため、重複を避けてスキップする。
            continue
...
def classify_csv_row(r):
    ...
    if not sp and dg:
        # OCRが話者名を本文頭に取り込んだ無言ビート（例「美嘉・心・きらり ・・・・」）を救済。
        # 空白より前が全て出演キャラ名で、残りが記号のみのときは、その名前を話者とするセリフ行にする。
...
def main():
    ...
    sections = [
        ...
        ("op", "OP", "OP", (9, 47), 125),
        ("ep1", "第1話", "1", (48, 110), 490),
        ("ep2", "第2話", "2", (111, 159), 1139),
        ("ep3", "第3話", "3", (160, 211), 1859),
        ("ep4", "第4話", "4", (212, 263), 2526),
        ("ep5", "第5話", "5", (264, 322), 3078),
        ("ed", "ED", "ED", (323, 361), 3638),
    ]
    ...
    # no1: 開催告知バナー, no2: 予告1カード, no3-4: みく(Tr1)
    #                       no5: 予告2カード, no6-8: 芳乃(Tr2)
    entries = []
