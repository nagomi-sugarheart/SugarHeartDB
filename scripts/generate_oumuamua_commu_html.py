# -*- coding: utf-8 -*-
"""merged_oumuamua_commu.json から オウムアムアに幸運を のイベントコミュHTMLを生成する

出力: scripts/oumuamua_commu_section.html（ページに手動で組み込む用の断片）
"""
import html
import json
import sys
...
MERGED = REPO / "scripts" / "merged_oumuamua_commu.json"
OUT = REPO / "scripts" / "oumuamua_commu_section.html"

CDN = "https://res.cloudinary.com/dnmzdghoi/image/upload/f_auto,q_auto/Deresute/Event/OumuamuaKooun"
...
LOG_LABELS = {"Tr1": "予告1", "Tr2": "予告2",
              "OP_B": "OP（動画ルート・青ボタン）", "OP": "OP（別ルート）",
...
def main():
    ...
    for i, sec in enumerate(data):
        ...
        tabs_html.append(
            f'        <button class="tab-item{active}" onclick="switchTab(this,\'oum-{sec["id"]}\')">{esc(sec["tab"])}</button>')
        ...
        rows_html = "\n        ".join(rows)
        panels_html.append(f'''    <div class="tab-panel{active}" data-tab="oum-{sec["id"]}">
