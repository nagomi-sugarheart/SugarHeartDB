# -*- coding: utf-8 -*-
"""merged_gojustgo_commu.json から Go Just Go！ のイベントコミュHTMLを生成する

出力: scripts/gojustgo_commu_section.html（ページに手動で組み込む用の断片）
"""
import html
import json
import sys
...
MERGED = REPO / "scripts" / "merged_gojustgo_commu.json"
OUT = REPO / "scripts" / "gojustgo_commu_section.html"

CDN = "https://res.cloudinary.com/dnmzdghoi/image/upload/f_auto,q_auto/Deresute/Event/GoJustGo"
...
LOG_LABELS = {"Tr1": "予告1", "Tr2": "予告2", "OP": "OP", "OP_B": "OP",
...
def main():
    ...
    for i, sec in enumerate(data):
        ...
        tabs_html.append(
            f'        <button class="tab-item{active}" onclick="switchTab(this,\'gjg-{sec["id"]}\')">{esc(sec["tab"])}</button>')
        ...
        rows_html = "\n        ".join(rows)
        panels_html.append(f'''    <div class="tab-panel{active}" data-tab="gjg-{sec["id"]}">
