# -*- coding: utf-8 -*-
"""SUN♡FLOWER イベントコミュのCSV×JSONマージスクリプト

- dialogues.csv: スクショのフレーム・タイムスタンプ・センターテキストを保持
- *_log.json:    完全なセリフテキスト・タイトル・要約を保持

両者を突合し、タブ生成用のマージ済みJSON (merged_sunflower_commu.json) と
突合レポート (merge_sunflower_report.txt) を出力する。
"""
import csv
import json
import re
...
CSV_PATH = Path(r"C:\Users\sawas\Downloads\comm_frames\SUN♡FLOWER_202607031942\dialogues.csv")
JSON_DIR = Path(r"G:\マイドライブ\コミュ")
JSON_PREFIX = "SUN♡FLOWER"
OUT_DIR = Path(__file__).parent
OUT_JSON = OUT_DIR / "merged_sunflower_commu.json"
OUT_REPORT = OUT_DIR / "merge_sunflower_report.txt"

# 場所・時間を示すセンターテキスト（タイトルカード扱い）
LOCATIONS = {
    "事務所", "カフェ", "翌日", "イベント当日", "海の家「SUN♡FLOWER」",
}
...
KNOWN_SPEAKERS = {"きらり", "美嘉", "未央", "早苗", "心", "P"}
...
def main():
    ...
    sections = [
        ...
        ("op", "OP", "OP", (10, 47), 101),
        ("ep1", "第1話", "1", (48, 85), 366),
        ("ep2", "第2話", "2", (86, 140), 711),
        ("ep3", "第3話", "3", (141, 190), 1091),
        ("ep4", "第4話", "4", (191, 236), 1437),
        ("ep5", "第5話", "5", (237, 279), 1757),
        ("ed", "ED", "ED", (280, 310), 2021),
    ]
    ...
    # no1: 開催告知バナー, no2: 予告1カード, no3-5: きらり(Tr1)
    #                       no6: 予告2カード, no7-9: 美嘉(Tr2)
    entries = []
