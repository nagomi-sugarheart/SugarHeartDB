# -*- coding: utf-8 -*-
"""Go Just Go！ イベントコミュのCSV×JSONマージスクリプト

- dialogues.csv: スクショのフレーム・タイムスタンプ・センターテキストを保持
- *_log.json:    完全なセリフテキスト・タイトル・要約を保持

両者を突合し、タブ生成用のマージ済みJSON (merged_gojustgo_commu.json) と
突合レポート (merge_gojustgo_report.txt) を出力する。

【本イベント固有の事情】
- OPにプロデューサーのセリフ選択による分岐があり、動画（CSV）は「青ボタン（右）」＝
  アニバーサリーLIVE「Go Just Go！」から始まるルート（OP_B ログ）を辿っている。
  もう一方の OP ログは1話「Sunshine in Summer！」と全58行が完全一致する複製
  （ログ取得時のミス）であり、意味のある分岐差分を持たない。よってOP本文は OP_B を
  使用し、複製である OP ログのテキスト・ログ画像は採用しない。
- OP・第1話・第2話・第3話・第5話には英語タイトルカードが映像に無い。各話先頭のフレーム
  （場所カードまたは先頭セリフ）を title_frame に使う。第4話・EDのみ英語タイトルカードあり。
"""
import csv
import json
import re
...
CSV_PATH = Path(r"C:\Users\sawas\Downloads\comm_frames\Go Just Go!_202606222303\dialogues.csv")
JSON_DIR = Path(r"G:\マイドライブ\コミュ")
JSON_PREFIX = "Go Just Go"
OUT_DIR = Path(__file__).parent
OUT_JSON = OUT_DIR / "merged_gojustgo_commu.json"
OUT_REPORT = OUT_DIR / "merge_gojustgo_report.txt"
...
SPEAKER_NORMALIZE = {
    ...
    "佐藤心": "心",
    "日野茜": "茜",
    "TVの中の瑞樹": "瑞樹",   # ED終盤のTV放送シーン
    "TVの中の心": "心",
}

# 出演キャラ（掛け合いのセンター話者判定・自己名ナレーション誤判定除外に使う）
KNOWN_SPEAKERS = {"唯", "加蓮", "志希", "心", "茄子", "響子", "瑞樹",
                  "りあむ", "愛海", "茜", "P", "一同"}
...
def main():
    ...
    sections = [
        ...
        ("op", "OP", "OP_B", (11, 70), 114, ["OP_B"]),
        ("ep1", "第1話", "1", (71, 127), 593, ["1"]),
        ("ep2", "第2話", "2", (128, 190), 1095, ["2"]),
        ("ep3", "第3話", "3", (191, 252), 1652, ["3"]),
        ("ep4", "第4話", "4", (253, 296), 2184, ["4"]),
        ("ep5", "第5話", "5", (297, 380), 2622, ["5"]),
        ("ed", "ED", "ED", (381, 428), 3187, ["ED"]),
    ]
    ...
    for code, card_no, (ra, rb) in [("Tr1", 2, (3, 5)), ("Tr2", 6, (7, 10))]:
