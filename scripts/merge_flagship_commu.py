...
# ログのメタデータ誤記を上書き（第3話ログの title/summary が第2話の複製）。
# title・summary はゲーム内スキップダイアログの公式表示に一致させた。
METADATA_OVERRIDES = {
    "3": {
        "title": "That is like waves",
        "summary": (
            "研究の成果を話し合う美穂たち。だが感情としては理解できても、"
            "実際にその気持ちになるのは難しく、行き詰まってしまう。"
            "通りかかった早苗と法子にも話を聞くが、答えは見つからない。"
            "それでも心はアイドルとして期待に応えようと、前向きに指針を示す。"
            "一方で加蓮は、ひとり思い詰めていた。"
        ),
    },
}
...
def main():
    ...
    # start_s はYouTubeチャプター（マージJSONの直前イベントtから算出）と一致させている。
    # op/ep3/ep4 は間の空行・UIプロンプト・末尾shotなしを挟むため、CSVの単純な直前行tでは
    # なくチャプター実測値（116/1694/2255）に合わせている。
    sections = [
        # (タブID, タブ名, JSONコード, CSV行範囲, 再生開始秒, ログ画像コード列)
        ("op", "OP", "OP", (9, 61), 116, ["OP"]),
        ("ep1", "第1話", "1", (62, 103), 611, ["1"]),
        ("ep2", "第2話", "2", (104, 156), 1124, ["2"]),
        ("ep3", "第3話", "3", (158, 209), 1694, ["3"]),
        ("ep4", "第4話", "4", (211, 266), 2255, ["4"]),
