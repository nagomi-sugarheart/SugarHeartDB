# -*- coding: utf-8 -*-
"""ダンシング・デッド コミュ画像のCloudinaryアップロード

- merged_dancingdead_commu.json で使用されているフレームのみアップロード
- ログ画像（Tr1/Tr2/OP/1〜5/ED）をアップロード（10MB超はJPEGに圧縮）
  ※ 当初 _5_log はEDの複製が保存されていたが、正しい第5話ログの提供を受けて
    差し替え済みのため、第5話ログも通常どおりアップロードする。
...
FRAMES_DIR = Path(r"C:\Users\sawas\Downloads\comm_frames\ダンシング・デッド_202606250743")
LOG_DIR = Path(r"G:\マイドライブ\コミュ")
LOG_PREFIX = "ダンシング・デッド"
MERGED = REPO / "scripts" / "merged_dancingdead_commu.json"
MAP_FILE = REPO / "_cloudinary_upload_map.json"
BASE = "Deresute/Event/DancingDead"
