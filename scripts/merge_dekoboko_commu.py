...
def main():
    ...
    sections = [
        ...
        ("trailer", "予告", ["Tr1", "Tr2"], (1, 9), 35),
        ("op", "OP", ["OP"], (10, 62), 115),
        ("ep1", "第1話", ["1"], (63, 115), 636),
        ("ep2", "第2話", ["2"], (116, 158), 1069),
        ("ep3", "第3話", ["3"], (159, 212), 1444),
        ("ep4", "第4話", ["4"], (213, 276), 1998),
        ("ep5", "第5話", ["5"], (277, 328), 2546),
        ("ed", "ED", ["ED"], (329, 369), 3182),
    ]
    ...
    for tab_id, tab_name, codes, (a, b), start_s in sections:
        ...
        if tab_id == "trailer":
            ...
            if tab_id == "op":
                # OPのJSONメタデータは4話の複製（誤記）。タイトルはCSVのタイトルカードから。
                # 要約はゲーム内原文が確認できないため、コミュ内容に基づく暫定要約。
                title = "結成！ふたりは魔法少女"
                summary = ("プロデューサーから新曲のプロモーション企画を一任された安部菜々と佐藤心。"
                           "菜々が幼い頃からずっと憧れ続けてきた「魔法少女」になる夢を叶えようと、"
                           "ふたりは魔法少女ステージ『しゅがしゅがハートでミンミンミン！ふたりは魔法少女☆』を企画する。"
                           "（※要約はゲーム内原文未確認のため暫定）")
