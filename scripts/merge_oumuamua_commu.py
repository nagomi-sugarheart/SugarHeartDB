# -*- coding: utf-8 -*-
"""オウムアムアに幸運を イベントコミュのCSV×JSONマージスクリプト

- dialogues.csv: スクショのフレーム・タイムスタンプ・センターテキストを保持
- *_log.json:    完全なセリフテキスト・タイトル・要約を保持

両者を突合し、タブ生成用のマージ済みJSON (merged_oumuamua_commu.json) と
突合レポート (merge_oumuamua_report.txt) を出力する。

【本イベント固有の事情】
- OPにプロデューサーのセリフ選択による分岐がある。動画（CSV）は「青ボタン（右）」＝
  Pが棒読みで「キレイダヨ」と応じるルートを辿っているため、OP本文は OP_B ログを使う。
  もう一方のルート（OP ログ）の心のセリフ差分は、注記(note)として本文に添える。
- OP・第1話・第3話・第5話には英語タイトルカードが映像に無い。各話先頭のフレーム
  （場所カードまたは先頭セリフ）を title_frame に使う。
"""
import csv
import json
import re
...
CSV_PATH = Path(r"C:\Users\sawas\Downloads\comm_frames\オウムアムアに幸運を_202606212358\dialogues.csv")
JSON_DIR = Path(r"G:\マイドライブ\コミュ")
JSON_PREFIX = "オウムアムアに幸運を"
OUT_DIR = Path(__file__).parent
OUT_JSON = OUT_DIR / "merged_oumuamua_commu.json"
OUT_REPORT = OUT_DIR / "merge_oumuamua_report.txt"

# 場所・シーンを示すセンターテキスト（タイトルカード扱い＝話者なしのシーン表示）
LOCATIONS = {
    "市庁舎内部", "休憩中", "乾杯！", "幸運を！",
}

# センターテキストのみでPのセリフとして扱うもの（Pの中央表示セリフ）
P_CENTER_LINES = {"ちゃんと見ているよ"}
...
# 撮影現場の5人＋劇中キャラ（カタカナ名）
KNOWN_SPEAKERS = {"ちとせ", "梨沙", "奈緒", "志希", "心", "P",
                  "チトセ", "リサ", "ナオ", "シキ", "シン"}
...
# OP分岐の別ルート注記（動画は青ボタン＝OP_Bルート）
OP_BRANCH_NOTE = (
    "このシーンはプロデューサーのセリフ選択で分岐します。動画は青（右）ボタン"
    "＝Pが棒読みで「キレイダヨ」と応じるルートです。もう一方のルートを選ぶと、"
    "直後の心のセリフが「はい……。じゃないっつーの☆ このテンプレやんの何回目だよ、オイ☆」に変化します。"
)


def insert_op_branch_note(section, report):
    """OPセクションの分岐セリフ直後に注記(note)を挿入する"""
    entries = section["entries"]
    for i, e in enumerate(entries):
        if e["type"] == "line" and "棒読みにもほどがある" in e["text"]:
            entries.insert(i + 1, {"type": "note", "speaker": "",
                                   "text": OP_BRANCH_NOTE, "frame": None, "t": None})
            report.append(f"[op] 分岐注記を挿入: 「{e['text'][:20]}」の直後")
            return
    report.append("[op] 分岐注記の挿入位置が見つかりませんでした")
...
def main():
    ...
    sections = [
        ...
        ("op", "OP", "OP_B", (10, 47), 130, ["OP_B", "OP"]),
        ("ep1", "第1話", "1", (48, 75), 521, ["1"]),
        ("ep2", "第2話", "2", (76, 123), 901, ["2"]),
        ("ep3", "第3話", "3", (124, 191), 1350, ["3"]),
        ("ep4", "第4話", "4", (192, 224), 1827, ["4"]),
        # ep5開始秒は、ep4末尾のナレーション（shotなし）を挟んだ本編開始位置に合わせて
        # 2168とする（YouTubeチャプターの補間値と一致させるため）。
        ("ep5", "第5話", "5", (225, 241), 2168, ["5"]),
        ("ed", "ED", "ED", (242, 289), 2547, ["ED"]),
    ]
    ...
    # no1: 開催告知バナー, no2: 予告1カード, no3-5: ちとせ(Tr1)
    #                      no6: 予告2カード, no7-9: 梨沙(Tr2)
    entries = []
    ...
    for code, card_no, (ra, rb) in [("Tr1", 2, (3, 5)), ("Tr2", 6, (7, 9))]:
    ...
    for tab_id, tab_name, code, (a, b), start_s, logs in sections:
        ...
        if first_typ in ("scene", "header"):
            ...
            # 先頭がセリフ（空フレーム含む） → 先頭フレームを見出し画像に使いつつ本文も突合
            title_frame = sec_rows[0]["frame_file"]
    ...
    for sec in result:
        if sec["id"] == "op":
            insert_op_branch_note(sec, report)
