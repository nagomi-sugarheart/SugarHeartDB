# -*- coding: utf-8 -*-
"""merged_stepandskip_commu.json から ステップ＆スキップ ゲストコミュのタブHTML断片を生成する

出力: scripts/stepandskip_commu_section.html（ページに組み込む用の <section> 断片）
"""
import html
import json
import sys
...
MERGED = REPO / "scripts" / "merged_stepandskip_commu.json"
OUT = REPO / "scripts" / "stepandskip_commu_section.html"

CDN = "https://res.cloudinary.com/dnmzdghoi/image/upload/f_auto,q_auto/Deresute/GuestCommu/StepAndSkip"
...
LOG_LABELS = {"OP": "OP", "1": "第1話", "2": "第2話", "3": "第3話",
              "4": "第4話", "5": "第5話", "ED": "ED"}
...
def main():
    ...
    for i, sec in enumerate(data):
        ...
        tabs_html.append(
            f'        <button class="tab-item{active}" onclick="switchTab(this,\'sas-{sec["id"]}\')">{esc(sec["tab"])}</button>')
        ...
        rows_html = "\n        ".join(rows)
        panels_html.append(f'''    <div class="tab-panel{active}" data-tab="sas-{sec["id"]}">
