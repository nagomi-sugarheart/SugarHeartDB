# -*- coding: utf-8 -*-
"""CoCo夏夏夏 Holiday イベントコミュのCSV×JSONマージスクリプト

- dialogues.csv: スクショのフレーム・タイムスタンプ・センターテキストを保持
- *_log.json:    完全なセリフテキスト・タイトル・要約を保持

両者を突合し、タブ生成用のマージ済みJSON (merged_coco_commu.json) と
突合レポート (merge_coco_report.txt) を出力する。

【本イベント固有の事情】
- OP分岐なし。全7セクション（OP・第1〜5話・ED）。
- 映像に英語タイトルカードは無い（Pleasant day 等）。各話先頭のフレーム
  （場所カードまたは先頭セリフ）を title_frame に使う。
- OP・第3話・EDは場所カードなしでセリフから始まる。OPと第3話は先頭の数行
  （3人「夏だーーーーっ！」等）が映像フレーム未取得のためshotなしで先頭に入る。
- 「後輩P」は主人公Pとは別人物（帯同する後輩プロデューサー）。Pへ正規化しない。
"""
import csv
import json
import re
...
CSV_PATH = Path(r"C:\Users\sawas\Downloads\comm_frames\CoCo夏夏夏Holiday_202606250743\dialogues.csv")
JSON_DIR = Path(r"G:\マイドライブ\コミュ")
JSON_PREFIX = "CoCo夏夏夏 Holiday"
OUT_DIR = Path(__file__).parent
OUT_JSON = OUT_DIR / "merged_coco_commu.json"
OUT_REPORT = OUT_DIR / "merge_coco_report.txt"

# 場所・時制を示すセンターテキスト（話者なしのシーン表示）
LOCATIONS = {
    "数か月前……", "出発日", "現在", "自然公園", "収録後",
    "打ち合わせ中", "十数分前……", "そして現在……",
    "ホテル", "シドニー", "LIVE当日",
}
...
KNOWN_SPEAKERS = {"心", "鈴帆", "愛梨", "梨沙", "亜季", "後輩P", "P",
                  "3人", "みんな"}
...
# CSVのOCRが話者欄を誤読した行の明示的な補正（no -> (speaker, dialogue)）
# no102: 話者が「プロデューサー」と誤読され、本文に実話者「3人」を取り込んでいる
ROW_FIXES = {
    102: ("3人", "瞑想？"),
}
...
DISPLAY_FIXES = [
    ("〜", "～"),
    ("はあと", "はぁと"),
    ("あつ。", "あっ。"),       # 3人の相槌「あっ。」のログOCR誤り
    ("今グテが終わった", "今ゲネが終わった"),  # なごみP「ゲネ（プロ）」のOCR誤り
    ("つくしゅん", "っくしゅん"),  # 亜季のくしゃみ（小書きっ）のOCR誤り
]
...
def classify_csv_row(r):
    ...
    if (sp and dg and ct and ct != dg and len(ct) <= 12
        ...
        # OCRが掛け合いの話者名をcenter_textに入れたケース（全要素が既知話者名のときのみ）
...
def main():
    ...
    for r in rows:
        fx = ROW_FIXES.get(int(r["no"]))
        if fx:
            r["speaker"], r["dialogue"] = fx
            r["center_text"] = ""
    ...
    sections = [
        ...
        ("op", "OP", "OP", (9, 66), 124, ["OP"]),
        ("ep1", "第1話", "1", (67, 117), 645, ["1"]),
        ("ep2", "第2話", "2", (118, 177), 1087, ["2"]),
        ("ep3", "第3話", "3", (178, 212), 1529, ["3"]),
        ("ep4", "第4話", "4", (213, 261), 2040, ["4"]),
        ("ep5", "第5話", "5", (262, 301), 2503, ["5"]),
        ("ed", "ED", "ED", (302, 369), 2874, ["ED"]),
    ]
    ...
    # no1: 開催告知バナー, no2: 予告1カード, no3-4: 心(Tr1)
    #                      no5: 予告2カード, no6-8: 鈴帆(Tr2)
    entries = []
