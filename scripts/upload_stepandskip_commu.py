# -*- coding: utf-8 -*-
"""ステップ＆スキップ ゲストコミュ画像のCloudinaryアップロード

- merged_stepandskip_commu.json で使用されているフレームのみアップロード
- ログ画像（Tr1/Tr2/OP_B/1〜5/ED）もアップロード（10MB超はJPEGに圧縮）
...
FRAMES_DIR = Path(r"C:\Users\sawas\Downloads\comm_frames\ステップ＆スキップ_202607162248")
LOG_DIR = Path(r"G:\マイドライブ\コミュ")
LOG_PREFIX = "ステップ＆スキップ"
MERGED = REPO / "scripts" / "merged_stepandskip_commu.json"
MAP_FILE = REPO / "_cloudinary_upload_map.json"
BASE = "Deresute/GuestCommu/StepAndSkip"
LOG_CODES = ["OP", "1", "2", "3", "4", "5", "ED"]
...
def resolve_log(code):
    """ログPNGの拡張子は大文字小文字が混在するため両対応で解決する"""
    for ext in (".png", ".PNG"):
