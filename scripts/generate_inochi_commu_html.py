# -*- coding: utf-8 -*-
"""merged_inochi_commu.json から 命燃やして恋せよ乙女 のイベントコミュHTMLを生成する

出力: scripts/inochi_commu_section.html（ページに手動で組み込む用の断片）
"""
import html
import json
import sys
...
MERGED = REPO / "scripts" / "merged_inochi_commu.json"
OUT = REPO / "scripts" / "inochi_commu_section.html"

CDN = "https://res.cloudinary.com/dnmzdghoi/image/upload/f_auto,q_auto/Deresute/Event/InochiMoyashiteKoiseyoOtome"
...
def main():
    ...
    for i, sec in enumerate(data):
        ...
        tabs_html.append(
            f'        <button class="tab-item{active}" onclick="switchTab(this,\'inochi-{sec["id"]}\')">{esc(sec["tab"])}</button>')
        ...
        rows_html = "\n        ".join(rows)
        panels_html.append(f'''    <div class="tab-panel{active}" data-tab="inochi-{sec["id"]}">
