# -*- coding: utf-8 -*-
"""merged_sunflower_commu.json から SUN♡FLOWER のイベントコミュHTMLを生成する

出力: scripts/sunflower_commu_section.html（ページに手動で組み込む用の断片）
"""
import html
import json
import sys
...
OUT = REPO / "scripts" / "sunflower_commu_section.html"

CDN = "https://res.cloudinary.com/dnmzdghoi/image/upload/f_auto,q_auto/Deresute/Event/SunFlower"
...
def main():
    ...
    for i, sec in enumerate(data):
        ...
        tabs_html.append(
            f'        <button class="tab-item{active}" onclick="switchTab(this,\'sun-{sec["id"]}\')">{esc(sec["tab"])}</button>')
        ...
        rows_html = "\n        ".join(rows)
        panels_html.append(f'''    <div class="tab-panel{active}" data-tab="sun-{sec["id"]}">
